...
private:
    ...
    void handleMessage(const std::shared_ptr<asio::ip::tcp::socket>& userSocket, const std::string& username, const std::string& message);

private:
    asio::io_context io_context_;
    asio::ip::tcp::acceptor acceptor_;
    unsigned short port_;
    std::map<std::shared_ptr<asio::ip::tcp::socket>, std::string> connectedUsers_;
